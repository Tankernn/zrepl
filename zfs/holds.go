...
func ZFSReleaseAllOlderAndIncludingGUID(ctx context.Context, fs string, snapGuid uint64, tag string) error {
	return doZFSReleaseAllOlderAndIncOrExcludingGUID(ctx, fs, snapGuid, tag, true)
}

// Idempotent: if the hold doesn't exist, this is not an error
func ZFSReleaseAllOlderThanGUID(ctx context.Context, fs string, snapGuid uint64, tag string) error {
	return doZFSReleaseAllOlderAndIncOrExcludingGUID(ctx, fs, snapGuid, tag, false)
}

func doZFSReleaseAllOlderAndIncOrExcludingGUID(ctx context.Context, fs string, snapGuid uint64, tag string, includeGuid bool) error {
	...
	if !foundGuid {
		return fmt.Errorf("cannot find snapshot with guid %v", snapGuid)
	}
