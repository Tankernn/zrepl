...
func (p *Sender) HintMostRecentCommonAncestor(ctx context.Context, r *pdu.HintMostRecentCommonAncestorReq) (*pdu.HintMostRecentCommonAncestorRes, error) {
	...
	fs := r.GetFilesystem()
	mostRecent, err := sendArgsFromPDUAndValidateExists(ctx, fs, r.GetVersion())
	if err != nil {
	...
	err = SetReplicationCursor(ctx, fs, mostRecent)
...
func (s *Sender) Send(ctx context.Context, r *pdu.SendReq) (*pdu.SendRes, zfs.StreamCopier, error) {
	...
	// update replication cursor (FIXME legacy)
	if sendArgs.From != nil {
		...
		err = SetReplicationCursor(ctx, sendArgs.FS, sendArgs.From)
		if err == zfs.ErrBookmarkCloningNotSupported {
			getLogger(ctx).Debug("not creating replication cursor from bookmark because ZFS does not support it")
			// fallthrough
		} else if err != nil {
...
func (p *Sender) SendCompleted(ctx context.Context, r *pdu.SendCompletedReq) (*pdu.SendCompletedRes, error) {
	...
	err = SetReplicationCursor(ctx, fs, to)
	if err != nil {
	...
	go func() {
		defer wg.Done()
		log.Debug("release step hold or bookmark on `to`")
		err = ReleaseStep(ctx, fs, to, p.jobId)
		if err != nil {
			log.WithError(err).Error("cannot release step holds or bookmarks on `to`")
		} else {
			log.Info("successfully released holds or bookmarks on `to`")
		}
	...
	go func() {
		defer wg.Done()
		if from != nil {
			log.Debug("release step hold or bookmark on `from`")
			err := ReleaseStep(ctx, fs, from, p.jobId)
			if err != nil {
				log.WithError(err).Error("cannot release step holds or bookmarks on `from`")
			} else {
				log.Info("successfully released holds or bookmarks on `from`")
			}
		}
	}()
	wg.Wait()
...
func (p *Sender) ReplicationCursor(ctx context.Context, req *pdu.ReplicationCursorReq) (*pdu.ReplicationCursorRes, error) {
	...
	cursor, err := GetReplicationCursor(dp)
	if err != nil {
		return nil, err
	}
	if cursor == nil {
